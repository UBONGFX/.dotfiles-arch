...
# --- quickshell poly bar: rebuild its colors.json from this theme's Waybar palette ---
# Same trail Waybar uses (colorschemes/<theme>/waybar/colors.css -> custom/<palette>.css),
# just emitted as JSON. Theme.qml watches the file, so there is nothing to restart.
polybar_colors="$HOME/Playground/poly-quickshell/apply-colors.sh"
[ -x "$polybar_colors" ] && "$polybar_colors" "$name" >/dev/null 2>&1 || true
...
# --- vscodium: colour theme + bracket-pair colours ---
# Not jq: settings.json is JSONC (trailing comma), which jq refuses to parse, so
# the old call failed silently on every switch. vscodium-apply.sh edits the two
# values in place instead and leaves the rest of the file untouched.
[ -x "$HOME/.config/colorschemes/vscodium-apply.sh" ] && \
    "$HOME/.config/colorschemes/vscodium-apply.sh" "$name" >/dev/null 2>&1 || true
